...
class ReportLogicHandler:
    ...
    def __init__(self, page_instance):
        # Hold the live report page so the exporter can read the currently displayed table state.
        self.page = page_instance

    def export_to_pdf(self):
        # Refuse export until the page has generated a report and table headings.
        if not self.page.current_headings:
            messagebox.showwarning("Export PDF", "Generate a report before exporting.")
            return

        # Import reportlab lazily so the page still loads even if PDF support is missing.
        try:
        ...
        # Configure the PDF page metrics shared by the header, table, and footer helpers.
        ...
        def draw_footer(page_no):
            # Stamp a simple branded footer and page number on every page.
        ...
        def draw_header():
            # Draw the report title bar and the visible summary cards at the top of the page.
        ...
        def draw_table_header(y_top, column_widths):
            # Paint the table heading row using the current report's column labels.
            header_h = 18
        ...
        def normalize_row(row):
            # Coerce each row into the exact column count expected by the PDF table.
        ...
        # Size the columns from the longest visible cell content before drawing the table body.
        ...
        # Render each visible row, paginating when the next row would exceed the bottom margin.
        for row in self.page.current_rows:
        ...
        # Finalize the last page and confirm where the export was saved.
        draw_footer(page_number)
        pdf.save()
...
def create_page(parent, user_info=None):
    # Delay importing the Tk page class until the module entrypoint is called.
