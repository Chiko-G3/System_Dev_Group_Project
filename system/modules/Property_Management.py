...
class AddApartmentStepper:
    ...
    def __init__(self, parent, refresh_callback, user_info=None):
        ...
        # Resolve role and city scope once so every wizard step uses the same access rules.
        ...
        # Admins skip the city picker because they are already scoped to one city.
    ...
    def step_city(self):
        ...
        # Render the first wizard card where non-admin users choose a city.
        ...
        # Advance to the address step with the currently selected city.
        # Next button
    ...
    def step_address(self, selected_city):
        ...
        # Fail fast on an invalid city so downstream building lookups stay consistent.
        ...
        # Rebuild the wizard card for the address selection step.
        # Create step container
        container = card(self.box_frame)
        styled_label(container, "Add Apartment", font=FONT_TITLE, fg="#222").pack(pady=(0, 4))
        tk.Frame(container, bg=ACCENT, height=3, width=60).pack(pady=(0, 16))
        ...
        # If the city has no buildings yet, stop here and offer a way back.
        ...
        # Carry both the city and selected building display label into the details step.
        # Next button
        btn_frame = tk.Frame(container, bg=BG)
        btn_frame.pack(pady=(20, 0))
        create_button(
    ...
    def _address_next(self, selected_city, selected_address):
        ...
        # Proceed only after a building address has been chosen.
    ...
    def step_details(self, city, address):
        ...
        # Render the final step where apartment-specific fields are collected.
        # Create step container
        container = card(self.box_frame)
        styled_label(container, "Add Apartment", font=FONT_TITLE, fg="#222").pack(pady=(0, 4))
        tk.Frame(container, bg=ACCENT, height=3, width=60).pack(pady=(0, 16))
        ...
        def on_type_change(event=None):
            # Keep studio units locked to one room while leaving other types editable.
        ...
        # Submit the fully collected wizard state to the apartment creation handler.
        # Submit button
        btn_frame = tk.Frame(container, bg=BG)
        btn_frame.pack(pady=(20, 0))
        create_button(
    ...
    def _submit(self, rooms, apt_type, occ, city, address):
        ...
        # Normalize raw widget values before validation and database lookup.
        # Trim inputs
        ...
        # Persist the apartment and then return control to the refreshed listing view.
...
def create_page(parent, user_info=None):
    ...
    # Keep module loading light by importing the UI page only when the module entrypoint is used.
