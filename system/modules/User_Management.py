...
class UserFormStepper:
    ...
    def __init__(self, parent, role_name_to_id, prefill=None):
        ...
        self._password_placeholder_active = False
    ...
    def _do_render(self, prefill=None):
        ...
        if prefill:
            ...
            self._set_existing_password_placeholder()
            role_name = prefill[4]
            if role_name in self.role_name_to_id:
                self.role_combobox.set(role_name)

    def _set_existing_password_placeholder(self):
        """Show a masked placeholder for existing passwords until the user edits it."""
        if self.password_entry is None:
            return

        self.password_entry.delete(0, tk.END)
        self.password_entry.insert(0, self.EXISTING_PASSWORD_MASK)
        self._password_placeholder_active = True
        self.password_entry.bind("<FocusIn>", self._clear_password_placeholder)

    def _clear_password_placeholder(self, _event=None):
        """Clear the masked placeholder the first time the password field is focused."""
        if not self._password_placeholder_active or self.password_entry is None:
            return
    ...
    def collect(self, require_password=False):
        ...
        if self._password_placeholder_active and password == self.EXISTING_PASSWORD_MASK:
            password = ""
