...
from main.helpers import (
    ...
    BG, ACCENT, FONT_TITLE, FONT_LABEL, set_treeview_empty_state
)
...
class ApartmentManagerPage:
    ...
    def __init__(self, parent, user_info=None):
        ...
        # Derive role and city scope once so the page can reuse them across actions.
        ...
        # Build the static page shell before loading the first apartment view.
        # Create main frame
    ...
    def create_buttons(self):
        """Create action buttons"""
        # Start from the shared property actions, then inject role-specific options.
        actions = [
    ...
    def refresh_apartments(self):
        ...
        if self.is_manager or self.is_admin:
            # Managers and admins get filter controls above the table.
            search_wrap = tk.Frame(self.box_frame, bg="#c9e4c4")
            search_wrap.pack(fill="x", pady=(0, 10))

            if self.is_manager:
                # Managers filter by city first, which then unlocks valid building choices.
        ...
        # Rebuild the apartment table from scratch for the latest dataset.
    ...
    def _refresh_property_building_filter_options(self):
        ...
        # Start from the unfiltered apartment source so the building list reflects the chosen city.
    ...
    def _apply_property_filters(self):
        ...
        # Apply each active UI filter in sequence, then re-render the table with the narrowed set.
        rows = list(self._source_apartments)
    ...
    def _render_apartment_rows(self):
        ...
        set_treeview_empty_state(self.apt_tree, bool(self._all_apartments))
    ...
    def show_update_status_stepper(self):
        ...
        # Build a dedicated status-management workspace instead of the standard apartment list.
        wrap = tk.Frame(self.box_frame, bg="#c9e4c4")
        ...
        def _sync_update_controls():
            # Hide the update controls for occupied units because those cannot be changed manually.
        ...
        def load_rows():
            # Reload the table from the selected status view while keeping role-based city scope.
        ...
        def apply_update():
            # Validate the selection, then persist the requested apartment status change.
            selected = tree.selection()
            if not selected:
    ...
    def show_add_city_stepper(self):
        ...
        # Render a small single-purpose form for creating a new city.
        ...
        # Submit function
        def submit_city():
            # Validate the typed name before inserting it and returning to the list view.
    ...
    def show_add_building_stepper(self):
        ...
        # Render the building form and preload the city choices available to this user.
        # Create form container
        container = card(self.box_frame)
        ...
        # Submit function
        def submit_building():
            # Resolve the chosen city, validate the form, then create the building record.
            if self.is_admin:
