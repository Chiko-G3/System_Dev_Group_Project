...
class Log_window:
    ...
    def _create_login_form(self):
        ...
        styled_label(pwd_container, "Password", fg="#555").pack(anchor="w", pady=(10, 2))

        password_row = tk.Frame(pwd_container, bg="white")
        password_row.pack(fill="x")

        password_border, self.password_entry = styled_entry(password_row)
        password_border.pack(side="left", fill="x", expand=True)
        ...
        self.toggle_btn.pack(side="left", padx=(8, 0), pady=(0, 1))
        self.toggle_btn.bind("<Button-1>", lambda _event: self._toggle_password_visibility())
        self._draw_password_toggle_icon()
    ...
    def _draw_password_toggle_icon(self):
        self.toggle_btn.delete("all")

        eye_color = "#3B86FF" if self._show_password else "#7f8c8d"
        self.toggle_btn.create_oval(6, 11, 30, 25, outline=eye_color, width=2)

        if self._show_password:
            self.toggle_btn.create_oval(15, 15, 21, 21, fill=eye_color, outline=eye_color)
        else:
            self.toggle_btn.create_line(10, 26, 27, 9, fill=eye_color, width=2)
