...
class ReportManagementPage:
    def __init__(self, parent, user_info=None):
        # Cache role and city scope so report queries can stay role-aware.
        ...
        # Build page state before the first report generation.
    ...
    def _build_layout(self):
        # Top area holds the report controls and the optional financial-only filters.
        ...
        # Snapshot cards summarize the currently selected report scope at a glance.
        ...
        # Financial reports can also render a lightweight paid-vs-pending chart below the table.
    ...
    def _render_financial_graph(self, rows):
        # Clear any previous chart before deciding whether the current report should show one.
        ...
        # Aggregate visible rows into paid and pending totals for the bar chart.
        for r in rows:
            try:
        ...
        def draw_metric(y, label, value, pct, color):
            # Draw one horizontal bar and its formatted amount/percentage label.
    ...
    def _update_summary_cards_visibility(self, report_key):
        ...
        # Each report type exposes only the summary cards that make sense for that view.
        visible_by_report = {
    ...
    def _load_city_filter_options(self):
        # Populate the city dropdown, or lock it to the user's assigned city when required.
    ...
    def _set_report_filter_visibility(self, report_key):
        ...
        # Financial reports swap the building selector for late/paid filters.
    ...
    def _resolve_city_id(self):
        # Convert the selected city label into the database id expected by report queries.
        if self.is_admin and self.assigned_city_id:
    ...
    def _refresh_building_filter_options(self):
        ...
        # Rebuild the building dropdown from the rows available to the current report type.
    ...
    def generate_report(self):
        # Refresh snapshot totals first so every report starts from the latest scoped summary data.
        city_id = self._resolve_city_id()
        ...
        if report_key == "Occupancy":
            # Occupancy reports are building-filterable and update the apartment count to match visible rows.
            rows = fetch_occupancy_rows(city_id)
            ...
            # Financial reports use late/paid filters and keep both the chart and summary card in sync.
            ...
            # Maintenance reports reuse the building selector to narrow visible request rows.
    ...
    def _render_table(self, title, columns, headings, widths, rows):
        # Replace the previous table entirely so the export state always matches what is on screen.
        ...
        if not rows:
            # Keep the page informative even when the current filters produce no data.
            tk.Label(
                self.table_host,
    ...
    def _generate_and_export(self):
        # Regenerate first so the PDF export uses the exact filters currently shown in the UI.
        self.generate_report()
