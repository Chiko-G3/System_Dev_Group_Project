...
from main.helpers import create_button, create_scrollable_treeview, set_treeview_empty_state, show_placeholder
...
class MaintenanceManagementPage:
    ...
    def _render_request_rows(self):
        ...
        set_treeview_empty_state(self.tree, bool(filtered_requests))
    ...
    def _approve(self, request_id=None):
        ...
        full_data = viewFull(target_id)
        if not full_data:
            messagebox.showerror("Error", f"Request #{target_id} was not found.", parent=self.frame)
            return

        if full_data[5] != "Open":
            messagebox.showerror("Error", "Only open requests can be approved.", parent=self.frame)
    ...
    def _deny(self, request_id=None):
        ...
        if full_data[5] != "Open":
            messagebox.showerror("Error", "Only open requests can be denied.", parent=self.frame)
            self._load_requests(reselect_id=target_id)
            self._on_row_select()
            return
