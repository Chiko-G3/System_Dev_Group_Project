...
from main.helpers import (
    ...
    create_date_input,
    set_treeview_empty_state,
    show_lease_removal_confirmation,
)
...
from validations import validate_lease_details
...
class LeaseManagerPage:
    ...
    def _render_leases(self, leases):
        ...
        set_treeview_empty_state(self.tree, bool(leases))
    ...
    def _build_layout(self):
        ...
        if self.user_role not in ["Administrators", "Manager"]:
    ...
    def _clear_form(self):
        ...
        self.start_entry.insert(0, self._default_start_date())
        self.end_entry.insert(0, self._default_end_date())
    ...
    def _build_form(self):
        ...
        start_wrapper, self.start_entry = create_date_input(
            self.form_frame,
            initial_value=self._default_start_date(),
        )
        start_wrapper.grid(row=1, column=1, padx=(0, 16), pady=8, sticky="we")

        self._form_field(self.form_frame, "End Date", 1, 2)
        end_wrapper, self.end_entry = create_date_input(
            self.form_frame,
            initial_value=self._default_end_date(),
            minimum_date=lambda: minimum_lease_end_date(
                self._get_form_start_date()
            ),
        )
        end_wrapper.grid(row=1, column=3, padx=(0, 16), pady=8, sticky="we")
    ...
    def _get_form_start_date(self):
        try:
            return date.fromisoformat(self.start_entry.get().strip())
        except Exception:
            return date.today()
    ...
    def _remove_lease(self):
        ...
        penalty = round(float(rent_str) * 0.05, 2)
        vacate = minimum_lease_end_date(date.today()).isoformat()
