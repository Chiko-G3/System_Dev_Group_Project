...
def validate_password_strength(password, required=True):
    """Validate password strength for staff user accounts."""
    normalized = "" if password is None else str(password).strip()
    ...
    if len(normalized) < 8:
        raise ValueError(
            "Password is too weak. Use at least 8 characters and any 3 of: uppercase letter, lowercase letter, number, special character."
        )

    strength_checks = [
        re.search(r"[A-Z]", normalized) is not None,
        re.search(r"[a-z]", normalized) is not None,
        re.search(r"\d", normalized) is not None,
        re.search(r"[^A-Za-z0-9\s]", normalized) is not None,
    ]
...
def validate_user_form(first_name, surname, email, role_name, require_password=False, password=""):
    ...
    if require_password or password:
        try:
            validate_password_strength(password, required=require_password)
...
def validate_iso_date(value, field_name):
    """Validate a single ISO date field and return it as a date object."""
...
def add_months_to_date(base_date, months):
    """Add months while preserving the day-of-month where possible."""
...
def minimum_lease_end_date(start_date):
    """Return the earliest valid lease end date for a given start date."""
...
def validate_lease_details(start_date, end_date, rent):
    ...
    if start_date:
        try:
            start_dt = validate_iso_date(start_date, "Start date")
        except ValueError as exc:
            errors.append(f"• {exc}")

    if end_date:
        try:
            end_dt = validate_iso_date(end_date, "End date")
        except ValueError as exc:
            errors.append(f"• {exc}")
    ...
    if end_dt and end_dt < today:
        errors.append("• End date cannot be before today")

    if start_dt and end_dt and end_dt < start_dt:
        errors.append("• End date must be on or after start date")

    if start_dt and end_dt and end_dt < minimum_lease_end_date(start_dt):
        errors.append("• End date must be at least 1 month after start date")
