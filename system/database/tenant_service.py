...
def normalize_tenant_email(email):
    """Normalize tenant email addresses for uniqueness checks."""
    return str(email).strip().lower()


def ensure_unique_tenant_email(email, exclude_tenant_id=None):
    """Raise when a tenant email already belongs to another tenant."""
    ...
    existing_tenant = execute_query(query, tuple(params), 'one')
    if existing_tenant is not None:
        raise ValueError("Tenant email already exists.")

    return normalized_email


def normalize_national_insurance_number(ni_number):
    """Normalize NI numbers so duplicate checks ignore spaces and letter case."""
    return "".join(str(ni_number).strip().upper().split())


def ensure_unique_national_insurance_number(ni_number, exclude_tenant_id=None):
    """Raise when an NI number already belongs to another tenant."""
    normalized_ni_number = normalize_national_insurance_number(ni_number)

    query = """
        SELECT tenant_id
        FROM Tenant
        WHERE UPPER(REPLACE(TRIM(ni_number), ' ', '')) = ?
    """
    params = [normalized_ni_number]
    ...
    existing_tenant = execute_query(query, tuple(params), 'one')
    if existing_tenant is not None:
        raise ValueError("National Insurance Number already exists for another tenant.")

    return normalized_ni_number


def ensure_tenant_contact_schema():
    """Ensure tenant records store contact emails directly and have no login credentials."""
    conn = check_connection()
    cursor = conn.cursor()

    try:
        try:
            cursor.execute("ALTER TABLE Tenant ADD COLUMN email VARCHAR(255)")
        except Exception:
            pass
        ...
        cursor.execute(
            ...
            SET email = lower(
                replace(
                    COALESCE(
                        (SELECT trim(u.first_name) FROM User u WHERE u.user_id = Tenant.user_id),
                        'tenant'
                    ),
                    ' ',
                    ''
                ...
                replace(
                    COALESCE(
                        (SELECT trim(u.surname) FROM User u WHERE u.user_id = Tenant.user_id),
        ...
        cursor.execute(
            """
            UPDATE Tenant
            SET ni_number = UPPER(REPLACE(TRIM(ni_number), ' ', ''))
            WHERE ni_number IS NOT NULL AND trim(ni_number) <> ''
            """
        )

        try:
            cursor.execute("CREATE UNIQUE INDEX idx_tenant_email ON Tenant(email)")
        except Exception:
            pass

        try:
            cursor.execute("CREATE UNIQUE INDEX idx_tenant_ni_number ON Tenant(ni_number)")
        except Exception:
            pass
...
def create_tenant(first_name, surname, email, telephone, reference_name, reference_email,
    ...
    ensure_tenant_contact_schema()
    normalized_email = ensure_unique_tenant_email(email)
    normalized_ni_number = ensure_unique_national_insurance_number(ni_number)
    ...
    try:
        ...
        cursor.execute("""
            INSERT INTO Tenant (user_id, ni_number, telephone, occupation, email)
            VALUES (?, ?, ?, ?, ?)
        """, (new_user_id, normalized_ni_number, telephone, occupation, normalized_email))
...
def update_tenant(tenant_id, first_name, surname, email, telephone, reference_name, reference_email,
    ...
    ensure_tenant_contact_schema()
    normalized_email = ensure_unique_tenant_email(email, exclude_tenant_id=tenant_id)
    normalized_ni_number = ensure_unique_national_insurance_number(ni_number, exclude_tenant_id=tenant_id)
    ...
    operations = [
        ...
        ("UPDATE Tenant SET ni_number = ?, telephone = ?, occupation = ?, email = ? WHERE tenant_id = ?",
            (normalized_ni_number, telephone, occupation, normalized_email, tenant_id)),
