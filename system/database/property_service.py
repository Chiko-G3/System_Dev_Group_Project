...
def ensure_unique_apartment(city_id, building_id, num_rooms, apt_type):
    """Block duplicate apartments within the same building using the fields the current schema stores."""
    existing = execute_query(
        ...
        LIMIT 1
        """,
        (building_id, num_rooms, apt_type),
        'one'
    )

    if existing is not None:
        raise ValueError(
            "Duplicate apartment blocked. An apartment with the same building, room count, and type already exists."
        )


def create_apartment(city_id, building_id, num_rooms, apt_type, occupancy_status):
    ensure_unique_apartment(city_id, building_id, num_rooms, apt_type)
    insert(
