...
import random
from .db_utils import execute_query, execute_transaction, is_manager, get_user_city_id, build_city_filter
...
def update_request_status(request_id, action):
    ...
    current_status_row = execute_query(
        "SELECT Maintenance_status FROM Maintenance_Request WHERE request_id = ?",
        (request_id,),
        'one'
    )
    ...
    if current_status not in allowed_current_status[new_status]:
        return False
