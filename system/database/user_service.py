...
from validations import validate_email_address, validate_password_strength
...
def create_user(first_name, surname, email, password_hash, role_id, city_id=None, scope_city_id=None):
    ...
    validate_password_strength(password_hash)
...
def update_user(user_id, first_name, surname, email, role_id, city_id=None, password_hash=None, scope_city_id=None):
    email = validate_email_address(email)
    if password_hash:
        validate_password_strength(password_hash, required=False)
...
def check_user(email, password):
    ...
    row = execute_query("""
        SELECT ua.password_hash, u.first_name, u.surname, r.role_name
        FROM User_Access ua
    ...
    stored_password, first_name, surname, role_name = row
    if role_name == "Tenant":
        return None
