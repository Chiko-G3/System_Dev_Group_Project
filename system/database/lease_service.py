...
def create_lease(apartment_id, tenant_id, start_date, end_date, agreed_rent):
    ...
    if end_dt < minimum_lease_end_date(start_dt):
        raise ValueError("End date must be at least 1 month after start date.")
...
def update_lease_early_termination(lease_id, fee):
    ...
    execute_query(
        "UPDATE Lease SET end_date = ?, early_termination_fee = ? WHERE lease_id = ?",
        (vacate_by, fee, lease_id),
        'none'
    )